...
export default function GlobalChatPage() {
  ...
  useEffect(() => {
    // 1. Load messages from 'cht' table
    ...
    return () => { pusher.unsubscribe('global-chat'); };
  }, []);
  ...
  const sendMessage = async (e: React.FormEvent) => {
    ...
    try {
      const res = await fetch('/api/chat/send', {
        ...
        body: JSON.stringify({ text: currentText, user_name: "Shivam" }),
      });
        ...
        {messages.map((m, i) => (
          <div key={i} className="flex items-start gap-3">
            ...
            <div className="bg-white p-3 rounded-2xl shadow-sm border border-slate-100 max-w-[80%]">
              ...
              <p className="text-sm text-slate-700">{m.chat}</p> {/* */}
            </div>
          </div>
        ))}
      </div>
      <form onSubmit={sendMessage} className="p-4 bg-white border-t flex gap-2">
        <input value={input} onChange={(e) => setInput(e.target.value)} className="flex-1 bg-slate-100 px-4 py-2 rounded-xl outline-none" placeholder="Say something..." />
        <button type="submit" className="bg-orange-500 text-white p-3 rounded-xl hover:bg-orange-600"><Send size={18} /></button>
      </form>
    </div>
  );
}